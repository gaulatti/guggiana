...
const buildPollyWorkflow = (
  ...
  const stateMachine = new StateMachine(stack, `${stack.stackName}ContentToSpeechStateMachine`, {
    definitionBody: DefinitionBody.fromString(JSON.stringify(languagesStateTemplate)),
    role: stateMachineRole,
  });

  stateMachineRole.addManagedPolicy(ManagedPolicy.fromAwsManagedPolicyName('AmazonPollyFullAccess'));
